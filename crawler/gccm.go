...
func gccm(maxGoRoutines int) *concurrencyManager {
...
// Create the controller to collect all the jobs. When a goroutine is finished
// we can release a slot for another goroutine.
func (c *concurrencyManager) controller() {
	for {
		...
		// When the closed flag is set, we need to close the manager if it
		// doesn't have any running goroutine.
...
// Wait until a slot is available for the new goroutine. A goroutine have to
// start after this function.
func (c *concurrencyManager) Wait() {

	// Try to receive from the manager channel. When we have something, it means
	// a slot is available and we can start a new goroutine. Otherwise, it will
